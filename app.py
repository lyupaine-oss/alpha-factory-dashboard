...
from filters import render_risk_slider

# 💡 【核心融合】引入刚刚定稿的因子挖掘模块
from factor_miner import render_factor_mining_page
...
# ============================================================
# 2. 侧边栏：主导航切换 + 订阅状态 + 更新时间
# ============================================================
st.sidebar.header("🧭 系统主导航")
# 🎯 接入多页面路由分流
...
# ============================================================
# ⚡ 路由分支 [2/2]：1768 因子大淘金分流点
# ============================================================
if nav_page == "🧬 1768 因子大淘金":
    # 直接调用 factor_miner.py 中封装好的核心渲染逻辑
...
# ============================================================
    ...
    # ★ 风控滑块（专家方案接入点）
    risk_threshold = render_risk_slider()
    ...
    with tab_signal:
        ...
        if df_signals is not None and not df_signals.empty:
            if 'risk_level' in df_signals.columns:
                df_filtered = df_signals[df_signals['risk_level'] <= risk_threshold].copy()
            else:
                df_filtered = df_signals.copy()

            hidden_count = len(df_signals) - len(df_filtered)
            if hidden_count > 0:
                st.info(f"🛡️ 风控已屏蔽 **{hidden_count}** 个高风险信号（风险水位 > {risk_threshold:.0f}）")

            long_signals  = df_filtered[df_filtered["direction"] == "多头"].sort_values("score", ascending=False).reset_index(drop=True)
            short_signals = df_filtered[df_filtered["direction"] == "空头"].sort_values("score", ascending=False).reset_index(drop=True)
        else:
        ...
        with col_long:
            ...
            for i, row in visible.iterrows():
                risk_tag = f"　风险：`{row['risk_level']:.0f}`" if 'risk_level' in row else ""
                st.write(f"**{i+1}.** {row['symbol']}　因子强度：`{row['score']:.1f}`{risk_tag}")
        ...
        if user_plan in ("pro", "pro_plus"):
            ...
            c1, c2 = st.columns(2)
            with c1:
                display_df = long_signals.head(10).assign(建议持有="5 交易日")
                show_cols  = [c for c in ['symbol', 'sector', 'score', 'risk_level', '建议持有'] if c in display_df.columns]
                st.dataframe(display_df[show_cols], use_container_width=True)
            with c2:
                display_df = short_signals.head(10).assign(建议持有="5 交易日")
                show_cols  = [c for c in ['symbol', 'sector', 'score', 'risk_level', '建议持有'] if c in display_df.columns]
                st.dataframe(display_df[show_cols], use_container_width=True)
        else:
    ...
    with tab_pnl:
        ...
        if df_pnl_cur is not None and not df_pnl_cur.empty:
            ...
            display_cols = [col for col in ["symbol", "name", "direction", "entry_price",
                                            "latest_price", "ret_pct", "days_held",
                                            "days_remaining", "exit_date"] if col in df_pnl_cur.columns]
    ...
    with tab_dict:
        ...
        factor_dict = {
            ...
            "黑色": {
                ...
                "free": True,
                "desc": "巨量持仓沉淀代表多空死磕，黑色系典型趋势突破催化剂。持仓近 historical 高位→一方被迫离场→动量加速启动。"
            },
            "有色": {
...
st.markdown("---")  # 在网页底部画一条优雅的分割线
st.caption("© 2026 Alpha Factory. All Rights Reserved.")
st.caption("系统说明：信号由多因子复合模型生成，每日 19:30 自动更新。")
st.caption("免责声明：本站所有内容仅供量化研究与技术交流参考，不构成任何投资建议。历史业绩不代表未来表现。据此操作风险自担。")