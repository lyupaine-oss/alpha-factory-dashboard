...
for symbol, cfg in CURRENT_POSITIONS.items():
    try:
        ...
        days_remain = max(0, sum(1 for d in all_dates if latest_date < d <= exit_date))
...
# 7. 平仓日归档
# ============================================================
is_exit_day = any(r.get("days_remaining", 0) == 0 for r in records)
...
if records:
    ...
    lines.append(f"⏱ 已持有 {records[0]['days_held']} 个交易日，剩余 {records[0]['days_remaining']} 个交易日")
else:
    lines.append("暂无持仓记录")
...
lines.append("")
lines.append("———")
lines.append("系统采用5日换仓机制，每日16:30自动更新。")
